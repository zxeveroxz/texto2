...
app.grid_columnconfigure(0, weight=1)  # Hacer que la columna 0 se expanda horizontalmente

# Utilizar el tema visual de Windows 10 u 11
style = ttk.Style(app)
style.theme_use('vista')  # O 'vista' para un tema más antiguo

# Crear un frame principal que ocupará todo el espacio de la ventana
main_frame = ttk.Frame(app)
main_frame.grid(row=0, column=0, sticky='nsew')

# Crear un widget de Canvas para contener el frame
canvas = tk.Canvas(main_frame, bg='white')
canvas.grid(row=0, column=0, sticky='nsew')  # Hacer que el Canvas ocupe todo el espacio disponible
...
# Crear un widget de desplazamiento vertical para el canvas
scrollbar_y = ttk.Scrollbar(main_frame, orient=tk.VERTICAL, command=canvas.yview)
scrollbar_y.grid(row=0, column=1, sticky='ns')  # Ubicar el Scrollbar a la derecha del Canvas

# Vincular el widget de desplazamiento al canvas
